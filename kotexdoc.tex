...
예를 들면 색인을 만들기 위한 \util{komkindex}와 \util{kotexindy} 등이 포함되어 있다.
\end{enumerate}
...
\item [cjk-\ko] \wi{김도현} (LPPL, maintained)
...
\pkg{untype1}을 제외한 모든 패키지가 LPPL 라이선스를 가진다.\footnote{참고로, untype1은 GPL이다.}
...
\kotex\ 패키지군 가운데 2013년 9월 현재 CTAN에 등재되어 있는 것은
...
패키지는 설치가 가능하다. 즉 현재 \pkg{cjk-ko}, \pkg{xetexko}, \pkg{luatexko}는
MiK\TeX 의 패키지매니저를 이용하여 설치 운용할 수 있다.\index{MiKTeX}

\util{tlmgr}이 없는 텍 배포판이라도 \TeX\,Live를 바탕으로 하고 있다면 마찬가지로 위의 세 패키지를 운용하는 것은
...
\util{kotexindy}가 함께 제공되는데, \XeTeX 이나 \LuaTeX 과
